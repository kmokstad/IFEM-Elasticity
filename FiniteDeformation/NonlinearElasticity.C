...
void NonlinearElasticity::printLog () const
{
  IFEM::cout <<"NonlinearElasticity: Total Lagrangian formulation "
				   ...
				   const Vec3& X) const
{
  ...
  Matrix Cmat;
  SymmTensor S(nsd);
  if (!this->formTangent(Cmat,S,fe,X,F,E))
    return false;
...
bool NonlinearElasticity::evalSol (Vector& s, const Vectors& eV,
                                   const FiniteElement& fe,
                                   const Vec3& X, bool toLocal, Vec3*) const
{
  ...
  if (!this->formStressTensor(eV.front(),fe,X,Sigma))
    return false;

  // Congruence transformation to local coordinate system at current point
  if (toLocal && locSys)
    Sigma.transform(locSys->getTmat(X));
					    ...
					    SymmTensor& S) const
{
  ...
  Matrix Cmat;
  double U;
  return material->evaluate(Cmat,S,U,fe,X,F,E,2);
}
...
{
  double U;
  return material->evaluate(Ctan,S,U,fe,X,F,E, E.isZero(1.0e-16) ? 0 : 2);
}
