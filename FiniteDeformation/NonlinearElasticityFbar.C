...
public:
  ...
  explicit FbarNorm(LocalIntegral& n) { myNorm = static_cast<ElmNorm*>(&n); }
  //! \brief Alternative constructor allocating an internal ElmNorm.
  explicit FbarNorm(size_t n) { myNorm = new ElmNorm(n); }
...
void NonlinearElasticityFbar::printLog () const
{
  IFEM::cout <<"NonlinearElasticityFbar: F-bar formulation, "<< npt1
							  ...
							  bool neumann) const
{
  ...
  for (Matrix& A : result->A)
    A.resize(nsd*nen,nsd*nen);
				       ...
				       const Vec3& X) const
{
  ...
  if (!material->evaluate(Cmat,Sig,Jbar,fe,X,F,E,lHaveStrains,&prm))
    return false;
