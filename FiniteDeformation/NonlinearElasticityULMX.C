...
/*!
  \brief A struct with integration point data needed by
  NonlinearElasticityULMX::finalizeElement().
*/
...
struct ItgPtData
{
  ...
  double u[6];  //!< Parametric coordinates of current integratioin point
  double detJW; //!< Jacobian determinant times integration point weight
  //! \brief Default constructor.
  ItgPtData() : F(3), Fp(3), detJW(0.0) { memset(u,0,6*sizeof(double)); }
};
...
public:
  ...
  explicit MxNorm(LocalIntegral& n)
  {
  ...
  explicit MxNorm(size_t n)
  {
    myNorm = new ElmNorm(n);
    Hh = new Matrix();
  }

  //! \brief The destructor destroys the ElmNorm object and deletes \ref Hh.
  virtual ~MxNorm()
  {
    myNorm->destruct();
    delete Hh;
  ...
  //! \brief Returns the ElmNorm object to assemble into the global one.
  ElmNorm& ref() { return *myNorm; }
...
{
  nCS = nSV = 2; // This integrand needs both current and previous solution
}


void NonlinearElasticityULMX::printLog () const
{
  IFEM::cout <<"NonlinearElasticityULMX: Mixed formulation,"
             <<" discontinuous pressure, p="<< p << std::endl;

  this->NonlinearElasticityUL::printLog();
}
...
/*!
  This method mainly stores the integration point quantities that depends
  on the basis function values in the internal member \ref MxElmData::myData.
  The actual numerical integration of the tangent stiffness and internal forces
  is then performed by the  finalizeElement() method.
*/
...
{
  MxElmData* mx = nullptr;
  MxMats* mxMat = dynamic_cast<MxMats*>(&elmInt);
  if (mxMat)
    mx = mxMat;
  else if (!(mx = dynamic_cast<MxNorm*>(&elmInt)))
    return false;

  ItgPtData& ptData = mx->myData[mx->iP++];
  const Vectors& eV = elmInt.ref()->vec;
...
/*!
  This method is used to implement the multiple integration point loops
  within an element, required by the mixed formulation with internal modes.
  All data needed during the integration has been stored internally in the
  \a elmInt argument by the evalInt() method.
*/
					       ...
					       const TimeDomain& prm, size_t iG)
{
  ...
  for (const ItgPtData& pt : mx.myData)
  {
    std::cout <<"\n     X"   << ++iP <<" = "<< pt.X;
    std::cout <<"\n     detJ"<< iP <<"W = "<< pt.detJW;
    std::cout <<"\n     F"   << iP <<" =\n"<< pt.F;
  ...
  if (nPM == 1)
  {
    ...
    for (const ItgPtData& pt : mx.myData)
    {
      ...
      dNdxBar[0].add(pt.dNdx,dVol);
      if (axiSymmetry)
	for (j = 1; j <= nEN; j++)
	  dNdxBar[0](j,1) += pt.Nr(j)*dVol;
    }
    dNdxBar[0].multiply(1.0/h1);
  ...
  for (iP = 0; iP < nGP; iP++)
  {
    ...
    Sig.emplace_back(3);
    double U = 0.0;
    if (!material->evaluate(D[iP],Sig.back(),U,fe,pt.X,pt.F,
			    SymmTensor(0),1,&prm))
      return false;
    ...
    if (!Sig.back().isZero())
    {
      ...
      lHaveStress = true;
      double pres = Sig.back().trace() * pt.detJW/3.0;
      for (i = 1; i <= nPM; i++)
        Sigm(i) += pres * pt.Phi(i);
    }
  }
  ...
  for (iP = 0; iP < nGP; iP++)
  {
    ...
    if (eKm)
    {
      ...
      MixedMat::addVolumetricTerms(D[iP],Sig[iP],Bpres,Press);
#if INT_DEBUG > 0
      std::cout <<"NonlinearElasticityULMX::Dmat"<< iP+1 <<" ="<< D[iP];
#endif
...
#ifdef USE_FTNMAT
      if (nsd == 2)
        acckmx2d_(axiSymmetry,nEN,pt.Nr.ptr(),pt.dNdx.ptr(),
                  dNdxBar[iP].ptr(),D[iP].ptr(),mx.A[eKm-1].ptr());
      else
				      ...
				      LocalIntegral& elmInt)
{
  ...
  if (!static_cast<MxNorm&>(elmInt).init(Xc,nPt,p.p,p.nsd))
    return false;

  LocalIntegral* norm = const_cast<LocalIntegral*>(elmInt.ref());
  return this->NormBase::initElement(MNPC,*norm);
}
...
{
  LocalIntegral* norm = const_cast<LocalIntegral*>(elmInt.ref());
  return myProblem.initElementBou(MNPC,*norm);
}


/*!
  This method just forwards to the NonlinearElasticityULMX::evalInt() method.
  Since this class declares NonlinearElasticityULMX as friend, it can access
  all its data members. The actual norm integration is then performed by
  the finalizeElement() method.
*/
				  ...
				  const TimeDomain& prm, const Vec3& X) const
{
  ...
  return p.evalInt(elmInt,fe,prm,X);
}
...
{
  LocalIntegral* norm = const_cast<LocalIntegral*>(elmInt.ref());
  return this->ElasticityNormUL::evalBou(*norm,fe,X,normal);
}


/*!
  This method is used to implement the multiple integration point loops
  within an element required by the mixed formulation. It performs a subset
  of the tasks done by NonlinearElasticityULMX::finalizeElement().
*/
					  ...
					  const TimeDomain& prm, size_t iG)
{
  ...
  for (iP = 0; iP < nGP; iP++)
  {
    ...
    double U = 0.0;
    if (!p.material->evaluate(Cmat,Sig,U,fe,pt.X,Fbar,Eps,3,&prm,&pt.F))
      return false;
